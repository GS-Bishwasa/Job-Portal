        ...
        <div className='flex gap-2 mb-6 mt-3'>
          {
            isEdit ? <>
              ...
              <button onClick={e => setisEdit(false)} className='bg-green-100 border border-green-600 px-4 py-2 rounded-lg'>
                Save
              </button>
            ...
            {jobsApplied.map((job, index) => true ? (
              <tr>
                ...
                <td className='py-2 px-4 border-b'>
                  <span className={`${job.status === 'Accepted' ? 'bg-green-100 px-4 py-1.5 rounded' : job.status === 'Rejected' ? 'bg-red-100 px-4 py-1.5 rounded' : 'bg-yellow-100 px-4 py-1.5 rounded'}`}>{job.status}</span>
                  </td>
              </tr>
            ) : (null))}
          </tbody>
